...
static int load_file(const char *filename) {
  ...
  while (feof(file)==0) {
    ...
    char sid[16], name[12], m_name[4], last_name[12], dept[32], cohort[6], gpa[5];
    fscanf(file, "%s ", sid);
    /*fscanf(file, "%s ", cohort);
    fscanf(file, "%s ", name);
    fscanf(file, "%s ", m_name);
    fscanf(file, "%s ", last_name);
    /*fscanf(file, "%s %s %s", dept);*/
    fscanf(file, "%s ", cohort);
    fscanf(file, "%s ", gpa);
    s->sid = strdup(sid);
    /*s->cohort = (int) strdup(cohort);
    s->name = strdup(name);
    s->dept = strdup(dept);
    s->gpa = (int) strdup(gpa);*/
    fprintf(stdout, "%s ", s->sid);
    add_student(s);
    /*fprintf(stdout, "%s %d %s %s %f", s->sid, s->cohort, s->name, s->dept, s->gpa);*/
    free(s);
  }
  fclose(file);
